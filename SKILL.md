...
- **正文**：
  - 标题层级（h1-h6）
  - **大标题提升**：识别「一、背景」「二、xxx」「01 引言」「1.引言」「结语」等样式伪装的彩色标题条，提升为 `##` 二级标题。**仅限「数字直接连接/英文点」形态**（`01引言`、`1.引言`）；「数字+顿号/空格」（`1、xxx`）是正文编号列表，**不会**被误提升
  - **表格 → GFM 管道表格**（含单元格内加粗/斜体/图片引用）
  - 粗体、斜体
  - 列表（有序/无序，含「li 与子 ul 同层混排」「纯容器嵌套」两种公众号布局容错）
  - 链接
  - 图片（下载到本地 `images/`，**按 magic bytes 修正真实扩展名**，SVG/GIF 伪装不再存错）
  - 代码块（微信 `<code>` 每行一个元素 → 正确逐行输出；缩进 NBSP 还原为普通空格）、引用块
...
4. **微信长链风控**：带 `poc_token` 等参数的长链（`/s?__biz=…`）易被风控验证页拦截（表现为标题 `untitled`、无图片、无正文结构）。换 UA/Referer/真实浏览器均无效——**让用户提供 `/s/xxxxxx` 短链**。
5. **原文残留孤立 ``` 会吞内容**：公众号正文（尤其 Prompt 模板）里残留的孤立代码围栏，会让后续大段内容被并入一个巨型代码块。转换后建议校验 ``` 配对（发现跨度异常大的代码块即中招），回源 HTML 定位删除。
